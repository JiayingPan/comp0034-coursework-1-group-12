...
if __name__ == '__main__':
 ...
 df = pd.read_csv('all.csv', parse_dates=['utc'], index_col=['utc'])
...
def clean_data(df):
    ...
    df_min = df_min.drop(['Unnamed: 0', 'Unnamed: 0.1', 'location_x', 'location_y'], axis=1)
    df_max = df_max.drop(['Unnamed: 0', 'Unnamed: 0.1', 'location_x', 'location_y'], axis=1)
    df_avg = df_avg.drop(['Unnamed: 0', 'Unnamed: 0.1'], axis=1)
    df_min.rename(columns={'PM2.5': 'PM2.5 (min)', 'PM10': 'PM10 (min)', 'Total': 'Total (min)'}, inplace=True)
    df_max.rename(columns={'PM2.5': 'PM2.5 (max)', 'PM10': 'PM10 (max)', 'Total': 'Total (max)'}, inplace=True)
    df_avg.rename(columns={'PM2.5': 'PM2.5 (avg)', 'PM10': 'PM10 (avg)', 'Total': 'Total (avg)'}, inplace=True)
    dataframe = [df_min, df_max, df_avg]
    df_merged = reduce(lambda left, right: pd.merge(left, right, on=['utc'], how='outer'), dataframe)
    return df_merged
