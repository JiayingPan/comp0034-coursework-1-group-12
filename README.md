...
- Data needed from the data set:     
The questions are about the air quality variation on the time scale in different area. Hence, ‘utc’ is required as input to the date picker to select the start and end dates, ‘location’ is required as input to the area selection box to select the city, parameter names of ‘PM2.5’ and ‘PM10’ are required as input to the pollutant selection box, and values of PM2.5 and PM10 are required to plot the chart.
